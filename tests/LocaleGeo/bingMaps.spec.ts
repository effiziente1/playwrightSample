...
test.describe('Geo Location Test', () => {
    ...
    test('The bing maps is located to munich', {
        ...
        //This is the location label for the munich geolocation - using regex to match variations
        const geoNamePattern = /GermanyBavariaMunich \(District\)(?:Munich)?/;
        ...
        const assertionDescription = 'Geo name matches pattern';
        ...
        await bingMapsPage.addStepWithAnnotation(AnnotationType.Assert, assertionDescription, async () => {
            ...
            await expect(bingMapsPage.geoName.locator, assertionDescription).toHaveText(geoNamePattern);
        });
    });
});